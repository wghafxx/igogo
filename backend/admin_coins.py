...
def public(row):
    return {k: row.get(k) for k in ("id", "amount", "note", "status", "created_at", "completed_at")}
...
async def credit(db, session_id, request_id, amount, note, admin):
    ...
    try:
        ...
        if (op["session_id"], op["amount"], op["note"]) != (session_id, float(amount), note):
