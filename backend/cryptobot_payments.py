...
async def find_by_payload(gateway, order_id):
    """Recover an invoice created by the provider whose ID was never stored locally."""
    found = await gateway.call("getInvoices", {"fiat": "RUB", "count": 1000})
    items = found.get("items") if isinstance(found, dict) else None
    return next((item for item in items or [] if isinstance(item, dict) and item.get("payload") == order_id), None)


async def fetch_invoice(gateway, invoice_id):
    found = await gateway.call("getInvoices", {"invoice_ids": str(invoice_id)})
    items = found.get("items") if isinstance(found, dict) else None
...
async def create_invoice(db, gateway, user, request_id, amount_rub, asset, app_url):
    ...
    if await db.retired_payment_orders.find_one({"_id": order_id}):
        raise HTTPException(409, "Счёт отменён полным сбросом экономики. Создайте новый счёт")
    doc = await db.deposits.find_one({"id": order_id}, {"_id": 0})
    retry = doc is not None
    ...
    try:
        ...
        if doc.get("provider_invoice_id"):
            invoice = await fetch_invoice(gateway, doc["provider_invoice_id"])
        elif retry:
            # A previous attempt may have created the invoice and stopped before saving its ID.
            invoice = await find_by_payload(gateway, order_id)
        if invoice is None:
...
async def synchronize(db, gateway, doc, hint_invoice_id=None):
    ...
    if not doc.get("provider_invoice_id"):
        try:
            invoice = await fetch_invoice(gateway, hint_invoice_id) if hint_invoice_id else await find_by_payload(gateway, doc["id"])
        except ProviderError as error:
            raise payment_error(error) from None
        if invoice and invoice.get("payload") == doc["id"]:
...
async def handle_event(db, gateway, event):
    ...
    await synchronize(db, gateway, doc, hint_invoice_id=invoice["invoice_id"])
