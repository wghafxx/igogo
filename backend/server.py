...
class AdminCoinsIn(InputModel):
    request_id: uuid.UUID
    amount: Decimal = Field(gt=0, le=admin_coins.MAX_BALANCE, decimal_places=2)
...
async def chat_identity(request: Request) -> tuple:
    ...
    if session_id:
        user = await db.users.find_one({"session_id": session_id}, {"_id": 0})
        if user:
            return session_id, {**user, "registered": True}
...
async def chats_create(payload: ChatCreateIn, request: Request):
    ...
    if payload.kind == "deposit":
        ...
        pending = await db.deposits.count_documents({"session_id": owner, "status": {"$in": ["pending", "processing"]}})
        if pending >= 5:
            raise HTTPException(status_code=400, detail="У вас уже 5 заявок в ожидании")
        receiver = RECEIVERS[0]
        deposit = {
            "id": str(uuid.uuid4()), "session_id": owner, "nickname": profile.get("nickname"), "discord_id": profile.get("discord_id"),
            "roblox_nick": profile.get("roblox_nick"), "roblox_link": profile.get("roblox_link"),
    ...
    created = await chat.create_chat(db, owner, profile, payload.kind, payload.text, deposit)
...
async def admin_chats(request: Request, status: str = "open", q: Optional[str] = None):
    ...
    return await chat.admin_list(db, status, q)
...
async def _chat_owner_user(found: dict) -> Optional[dict]:
    if found.get("guest") or str(found.get("owner", "")).startswith("guest:"):
        return None
    u = await db.users.find_one({"session_id": found["owner"]}, {"_id": 0, "session_id": 1, "nickname": 1, "discord_id": 1, "roblox_nick": 1, "roblox_link": 1, "balance": 1, "skins.price": 1, "promo_code": 1, "promo_bonus": 1, "created_at": 1})
    if not u:
        return None
...
async def admin_chat_deposit(chat_id: str, payload: AdminConfirmIn, request: Request):
    ...
    if not dep:
        ...
        dep = {
            ...
            "roblox_nick": full.get("roblox_nick"), "roblox_link": full.get("roblox_link"),
    ...
    if not result.get("already_confirmed"):
        await chat.post_message(db, found, "system", f"Пополнение подтверждено: зачислено {float(result.get('credited') or 0):.2f} RAP (скины и остаток на балансе)")
...
async def admin_chat_withdrawals_done(chat_id: str, request: Request):
    ...
    if done:
        await chat.post_message(db, found, "system", f"Скины выданы: {done} шт. на {total:.2f} RAP. Проверьте инвентарь Roblox")
...
async def admin_player_coins(session_id: str, payload: AdminCoinsIn, request: Request):
    admin = await require_admin(request)
    return await admin_coins.credit(db, session_id, str(payload.request_id), payload.amount, payload.note.strip(), admin["jti"])


@api_router.get("/admin/players/{session_id}/coins")
async def admin_player_coin_history(session_id: str, request: Request):
    await require_admin(request)
    return [admin_coins.public(row) for row in await db.admin_coin_grants.find({"session_id": session_id}).sort("created_at", -1).to_list(10)]
...
async def admin_confirm_deposit(deposit_id: str, payload: AdminConfirmIn, request: Request):
    ...
    if not result.get("already_confirmed"):
        ...
        await chat.notify_deposit(db, dep, f"Пополнение подтверждено: зачислено {float(result.get('credited') or 0):.2f} RAP (скины и остаток на балансе)")
    return result
...
async def admin_reject_deposit(deposit_id: str, payload: AdminRejectIn, request: Request):
    ...
    if not res.matched_count:
        raise HTTPException(status_code=404, detail="Заявка не найдена или уже обработана")
    dep = await db.deposits.find_one({"id": deposit_id}, {"_id": 0, "chat_id": 1})
    await chat.notify_deposit(db, dep, f"Заявка на пополнение отклонена. Причина: {payload.reason}")
    return {"ok": True}
...
async def admin_withdrawal_cancel(withdrawal_id: str, payload: AdminRejectIn, request: Request):
    ...
    if w:
        await chat.notify_owner(db, w["session_id"], f"Вывод «{(w.get('item') or {}).get('name')}» отменён, скин возвращён в инвентарь. Причина: {payload.reason}")
    return result
...
async def shutdown_db_client():
    for name in ("referral_reconciliation", "xrocket_reconciliation", "cryptobot_reconciliation"):
