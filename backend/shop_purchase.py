...
# Receipts kept inside the user document for the atomic retry check; the full history lives in db.shop_receipts.
INLINE_RECEIPTS = 20


async def archive_receipts(db, session_id, receipts):
    for receipt in receipts:
        await db.shop_receipts.update_one({"_id": receipt["id"]}, {"$setOnInsert": {**receipt, "session_id": session_id}}, upsert=True)


async def purchase_skins(db, user, request_id, lines, expected_total):
    order_id = str(request_id)
    if order_id in user.get("retired_shop_orders", []):
        raise HTTPException(409, "Покупка отменена полным сбросом экономики. Обновите корзину")
    ...
    if receipt is None:
        archived = await db.shop_receipts.find_one({"_id": order_id, "session_id": user["session_id"]}, {"_id": 0, "session_id": 0})
        if archived:
            receipt = archived
    if receipt is None:
        # Everything that can be trimmed from the inline window below is archived first.
        await archive_receipts(db, user["session_id"], user.get("shop_receipts", []))
        ...
        fresh = await db.users.find_one_and_update(
            ...
            [{"$set": {
                ...
                "shop_receipts": {"$slice": [{"$concatArrays": [{"$ifNull": ["$shop_receipts", []]}, {"$literal": [receipt]}]}, -INLINE_RECEIPTS]},
            }}],
        ...
        user = fresh
        await archive_receipts(db, user["session_id"], [receipt])
