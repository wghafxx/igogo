...
@pytest.mark.parametrize("rubles, expected", [("45", 90), ("0.29", 0.58), ("2500000.25", 5000000.5)])
def test_ruble_credit_converts_on_server_and_retries_once(api, rubles, expected):
    async def check():
        payload = {"request_id": str(uuid.uuid4()), "amount_rub": rubles, "note": "Paid RUB"}
        results = await asyncio.gather(*(api.request("/admin/players/player/coins", "POST", json=payload) for _ in range(8)))
        assert all(r.status_code == 200 for r in results), [r.text for r in results]
        assert all(r.json()["amount"] == expected for r in results)
        user = await api.db.users.find_one({"session_id": "player"})
        assert user["balance"] == 12.5 + expected
        assert user["admin_coin_receipts"] == [payload["request_id"]]
        history = (await api.request("/admin/players/player/coins")).json()
        assert len(history) == 1 and history[0]["amount_rub"] == float(rubles)
        assert history[0]["amount"] == expected
        # A request ID cannot change units, even when the coin amount is equal.
        legacy = {"request_id": payload["request_id"], "amount": str(expected), "note": "Paid RUB"}
        assert (await api.request("/admin/players/player/coins", "POST", json=legacy)).status_code == 409
    run(check())


@pytest.mark.parametrize("fields", [
    {}, {"amount_rub": "45", "amount": "90"}, {"amount_rub": "0"},
    {"amount_rub": "-1"}, {"amount_rub": "NaN"}, {"amount_rub": "Infinity"},
    {"amount_rub": "1.001"}, {"amount_rub": "17592186044416.01"},
])
def test_invalid_ruble_credit_does_not_change_balance(api, fields):
    async def check():
        payload = {"request_id": str(uuid.uuid4()), **fields}
        response = await api.request("/admin/players/player/coins", "POST", json=payload)
