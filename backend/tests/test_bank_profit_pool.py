...
def test_pool_can_increase_decrease_and_reach_zero_without_changing_bank(bank_api):
    async def check():
        ...
        for amount, expected in ((100, 600), (-250, 350), (-350, 0)):
            r = await a.request("/admin/bank/pool", json={"amount": amount, "note": "test adjustment", "pin": "1001"})
...
def test_invalid_pool_change_does_not_write(bank_api, amount):
    async def check():
        a = bank_api
        r = await a.request("/admin/bank/pool", json={"amount": amount, "note": "test", "pin": "1001"})
...
def test_concurrent_pool_decreases_cannot_overdraw_or_use_active_luck(bank_api):
    async def check():
        a = bank_api
        await a.rain()
        results = await asyncio.gather(*(a.request("/admin/bank/pool", json={"amount": -300, "note": "test", "pin": "1001"}) for _ in range(2)))
...
def test_admin_profit_is_commission_only_and_adjustments_cannot_spend_it(bank_api):
    async def check():
        ...
        r = await a.request("/admin/bank/adjust", json={"amount": -801, "note": "test", "pin": "1001"})
        assert r.status_code == 400 and (await a.state())["bank"] == 1000
        r = await a.request("/admin/bank/adjust", json={"amount": 100, "note": "test", "pin": "1001"})
