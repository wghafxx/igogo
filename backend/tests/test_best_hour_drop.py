...
def test_best_drop_uses_the_whole_day_and_excludes_expired_or_future_wins(isolated_server):
    server = isolated_server
    server.LIVE_DROPS_TTL = 0
    ...
    rows += [drop("winner", 43200, 3000), drop("old", 86401, 99999), drop("future", -1, 999999)]
    ...
    assert feed["best_drop_expires_at"].startswith("2026-09-13T00:00:00")
    assert isinstance(asyncio.run(fetch(False)), list)
    rows[:] = [drop("old-only", 86401, 99999)]
