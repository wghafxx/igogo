    ...
    def matches(query):
        for key, value in query.items():
            if isinstance(value, dict):
                if "$in" in value and deposit.get(key) not in value["$in"]: return False
                if "$nin" in value and deposit.get(key) in value["$nin"]: return False
            elif deposit.get(key) != value: return False
        return True

    def find(query, projection):
        cursor = MagicMock()
        rows = [dict(deposit)] if matches(query) else []
        cursor.sort.return_value.to_list = AsyncMock(return_value=rows)
        cursor.sort.return_value.limit.return_value.to_list = AsyncMock(return_value=rows)
        return cursor

    collection.update_one = AsyncMock(side_effect=update)
    collection.find.side_effect = find
    collection.find_one = AsyncMock(side_effect=lambda *args: dict(deposit))
