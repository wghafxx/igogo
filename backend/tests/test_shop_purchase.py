...
def shop_api(isolated_server):
    ...
    async def atomic_debit(query, pipeline, **kwargs):
        ...
        account.setdefault("shop_receipts", []).extend(deepcopy(update["shop_receipts"]["$slice"][0]["$concatArrays"][1]["$literal"]))
    ...
    async def archive_write(query, update, **kwargs):
        archive.setdefault(query["_id"], deepcopy(update["$setOnInsert"]))

    async def archive_read(query, projection=None):
        row = archive.get(query["_id"])
        return {k: v for k, v in deepcopy(row).items() if k != "session_id"} if row and row["session_id"] == query["session_id"] else None

    server.db.shop_receipts = SimpleNamespace(update_one=AsyncMock(side_effect=archive_write), find_one=AsyncMock(side_effect=archive_read))
