...
def payment_api(isolated_server, monkeypatch):
    ...
    async def setup():
        await server.economy_guard.ensure(db)
        ...
        await db.users.insert_many([{"session_id": f"discord_{i}", "discord_id": str(i),
                                     "nickname": f"Player {i}", "balance": 0, "skins": [],
                                     "roblox_display_name": f"Player {i}", "roblox_nick": f"player_{i}", "roblox_link": f"https://www.roblox.com/users/{i}/profile"} for i in (1, 2)])
