  ...
  return (
    <div className="w-full space-y-2 text-[11px] text-[#b4b7c7]" data-testid={testId}>
      <div className="flex flex-wrap gap-x-4 gap-y-1">
        {!["xrocket", "cryptobot"].includes(deposit.payment_method) && <span data-testid={`${testId}-skins-total`}>{t("receipt.skins")} <b className="text-[#ffb000]">{skins.length} {t("receipt.pcs")} · {formatMoney(deposit.skins_total || 0)} RAP</b></span>}
      ...
      {skins.length > 0 && <div className={`grid gap-2 ${compact ? "grid-cols-2 sm:grid-cols-5" : "grid-cols-2 sm:grid-cols-3"}`}>
        {skins.map((skin) => (
          <div key={skin.uid || skin.id} className="rounded-lg bg-[#15161c] p-2 min-w-0 border-b-2" style={{ borderColor: rarityColor(skin.rarity) }} data-testid={`${testId}-skin-${skin.uid || skin.id}`}>
            {skin.image && <img src={skinImg(skin.image)} alt={`${skin.type} ${skin.name}`} loading="lazy" decoding="async" className="w-full aspect-[2/1] object-contain" />}
