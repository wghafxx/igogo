...
export default function Notifications() {
  ...
  return <Popover open={open} onOpenChange={(value) => { setOpen(value); if (value) refresh.current(); }}>
    <PopoverTrigger asChild>
      <AnimButton icon={BellIcon} size={18} className="relative w-9 sm:w-11 h-10 sm:h-11 flex items-center justify-center text-[#9a9db0] hover:text-white transition-colors" aria-label={`${t("header.notifications")}${unread ? `: ${unread} ${t("notifications.unread")}` : ""}`} title={t("header.notifications")} data-testid="notifications-button">
