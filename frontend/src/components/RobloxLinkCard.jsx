...
export default function RobloxLinkCard() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setAuthUser(u);
      setOpen(false);
      toast.success(t("profile.roblox_saved"));
    } catch (e) {
      const d = e?.response?.data?.detail;
      toast.error(typeof d === "string" ? d : t("profile.roblox_bad"));
    } finally {
      setBusy(false);
    }
  };
        ...
        <DialogContent className="bg-[#1e1f23] border-0 text-white sm:max-w-[460px] p-0 overflow-hidden rounded-2xl" data-testid="roblox-dialog">
          ...
          <div className="px-6 pt-4 pb-6 space-y-4">
            <div className="rounded-lg bg-[#00a2ff]/10 border border-[#00a2ff]/40 px-3 py-2.5 text-[12px] text-[#b4d9ff] leading-relaxed" data-testid="roblox-help">
              {t("profile.roblox_help")} <b>{t("profile.roblox_help_friends")}</b> {t("profile.roblox_help_mid")} <b>{t("profile.roblox_help_qr")}</b>{t("profile.roblox_help_tail")} <b>{t("profile.roblox_help_link")}</b>{t("profile.roblox_help_end")}
            </div>
            <label className="block space-y-1.5">
              <span className="text-[12px] text-[#8e91a3]">{t("profile.roblox_nick")}</span>
              <input value={nick} onChange={(e) => setNick(e.target.value)} placeholder={t("profile.roblox_nick_ph")} className="w-full h-11 px-3 rounded-lg bg-[#0f1015] outline-none text-[13px] font-bold focus:ring-1 focus:ring-[#00a2ff]" data-testid="roblox-nick-input" />
            </label>
            <label className="block space-y-1.5">
              <span className="text-[12px] text-[#8e91a3]">{t("profile.roblox_link")}</span>
