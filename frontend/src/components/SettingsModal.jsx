...
const NumInput = ({ prefix, suffix, value, onChange, testId }) => (
  <div className="h-9 w-[46px] shrink-0 rounded-md bg-white/5 border border-transparent flex items-center justify-center text-[13px] font-bold focus-within:border-white">
...
const RadioOption = ({ value, title, desc }) => (
  <label className="flex items-start gap-2 cursor-pointer">
    <RadioGroupItem value={value} data-testid={`settings-radio-${value}`} className="mt-0.5 border-[#777777] text-white data-[state=checked]:border-white" />
    <div>
...
export default function SettingsModal({ open, onOpenChange, settings, onSave }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="modal-surface border-0 text-white sm:max-w-[500px] p-0 max-h-[92dvh] overflow-y-auto gap-0" data-testid="settings-modal">
        ...
        <div className="px-6 pt-5 pb-6 space-y-5">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="m-box p-3">
                <div className="text-[11px] text-white/50 mb-3 leading-snug">{t("settings.mult_hint")}</div>
                <div className="flex items-center gap-1.5">
                  {draft.multipliers.map((m, i) => (
                    <NumInput key={i} prefix="x" value={m} onChange={(v) => setMult(i, v)} testId={`settings-mult-${i}`} />
                  ))}
                  <button data-testid="settings-reset-multipliers" aria-label={t("settings.reset_mult")} className="ml-auto text-white hover:rotate-[-90deg] transition-transform" onClick={() => setDraft((d) => ({ ...d, multipliers: DEFAULT_SETTINGS.multipliers }))}>
                    <RefreshCWIcon size={16} />
                  </button>
                </div>
              </div>
              <div className="m-box p-3">
                <div className="text-[11px] text-white/50 mb-3 leading-snug">{t("settings.pct_hint")}</div>
                <div className="flex items-center gap-1.5">
                  {draft.percents.map((p, i) => (
                    <NumInput key={i} suffix="%" value={p} onChange={(v) => setPct(i, v)} testId={`settings-pct-${i}`} />
                  ))}
                  <button data-testid="settings-reset-percents" aria-label={t("settings.reset_pct")} className="ml-auto text-white hover:rotate-[-90deg] transition-transform" onClick={() => setDraft((d) => ({ ...d, percents: DEFAULT_SETTINGS.percents }))}>
                    <RefreshCWIcon size={16} />
                  </button>
                </div>
              </div>
          ...
          <div className="m-foot !-mb-6">
            <button className="m-cta" onClick={save} data-testid="settings-save">
              {t("settings.save")}
            </button>
          </div>
        </div>
