...
const skins = Array.from({ length: 32 }, (_, i) => ({ id: `skin-${i}`, uid: `owned-${i}`, type: "AWP", name: `Skin ${i}`, price: 100 + i, rarity: "pink" }));
...
test("both lists have independent pages and retain selected skins", async () => {
  expect(document.querySelectorAll('[data-testid^="bet-card-"]')).toHaveLength(15);
  expect(document.querySelectorAll('[data-testid^="shop-skin-"]')).toHaveLength(15);
  ...
  expect(byId("bet-card-owned-15")).not.toBeNull();
  expect(byId("shop-skin-skin-0")).not.toBeNull();
  await click("shop-pagination-page-3");
  await load();
  expect(api.shop).toHaveBeenLastCalledWith({ sort: "price_desc", page: 3, limit: 15 });
...
test("search from a later page resets the catalog to page one", async () => {
  ...
  await load();
  expect(api.shop).toHaveBeenLastCalledWith({ sort: "price_desc", page: 1, limit: 15, q: "Skin 31" });
  expect(byId("shop-skin-skin-31")).not.toBeNull();
  expect(byId("shop-pagination")).toBeNull();
});

test("inventory returns to an available page when skins are removed", async () => {
  await click("inventory-pagination-page-3");
  await act(async () => { root.render(<Harness inventory={skins.slice(0, 16)} />); });
