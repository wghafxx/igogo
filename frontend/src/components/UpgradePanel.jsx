...
const SlotItem = ({ item, onRemove, testId, disabled, removeTitle }) => (
  <div className="slot-item" style={{ "--rarity": rarityColor(item.rarity) }} data-testid={testId}>
    ...
    <img src={skinImg(item.image)} alt={item.name} className="slot-item-img" decoding="async" draggable={false} />
    <div className="text-[13px] sm:text-[14px] font-bold text-white flex items-center justify-center gap-1 pb-3 sm:pb-5">
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <AnimButton
          ...
          aria-label={label}
          data-testid={testId}
          className={`w-6 h-6 flex items-center justify-center rounded transition-colors ${active ? "text-white bg-white/10" : "text-[#999999] hover:text-white hover:bg-white/5"}`}
        />
      </TooltipTrigger>
...
export default function UpgradePanel({ sessionId, user, settings, onSettingsChange, onOpenSettings, onUpgraded, onSpinningChange, target, onClearTarget, betSkins = [], onRemoveBetSkin }) {
  ...
  const [cfg, setCfg] = useState({ rtp: 0.85, min_chance: 0.01, max_chance: 0.75, max_bet_ratio: 0.75 });
  ...
  const RTP = cfg.rtp;
  const MAX_RATIO = Math.min(MAX_CHANCE, cfg.max_bet_ratio ?? MAX_CHANCE);
  ...
  const skinsCents = betSkins.reduce((sum, sk) => sum + Math.round((Number(sk.price) || 0) * 100), 0);
  const maxTotalCents = Math.floor(Math.round(targetPriceNum * 100) * MAX_RATIO + 1e-8);
  const maxBet = target && targetPriceNum > 0 ? Math.max(0, Math.min(Math.floor(balance * 100 + 1e-8), maxTotalCents - skinsCents)) / 100 : 0;
  const totalBet = bet + skinsTotal;
  const overLimit = Boolean(target) && targetPriceNum > 0 && Math.round(bet * 100) + skinsCents > maxTotalCents;
  ...
  return (
    <section className="game-interaction-surface animate__animated animate__fadeInUp" onDragStartCapture={preventGameDrag} data-testid="upgrade-panel">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-[1fr_300px_1fr] gap-2 sm:gap-3 lg:gap-x-6 lg:gap-y-3 items-stretch">
        <div className="blox-panel upgrade-preview-panel relative overflow-hidden h-[230px] sm:h-[300px] px-3 sm:px-4 pt-4 sm:pt-5 pb-3 sm:pb-4 flex flex-col order-2 lg:order-none" data-testid="source-panel">
          ...
          <div className="text-center">
            <div className="h-[30px] sm:h-[34px] flex items-center justify-center text-[12px] sm:text-[13px] font-bold leading-tight">{t("upgrade.source_title")}</div>
            <div className="text-[11px] text-[#7d8194] mt-1 flex items-center justify-center gap-1">
              <span className="inline-block min-w-[52px] text-right">{t("upgrade.bet")}</span> <RobuxIcon size={11} /> <span className="tabular-nums inline-block min-w-[40px] text-left" data-testid="total-bet">{formatMoney(totalBet)}</span>
            </div>
          </div>
          <div className="upgrade-slot slot-bg flex-1 flex items-center justify-center" data-testid="source-slot">
            ...
            <Logo size={92} muted testId="source-brand-icon" className="relative rotate-180 w-[64px] h-[64px] sm:w-[92px] sm:h-[92px]" />
          </div>
        </div>

        <div className="col-span-2 lg:col-span-1 flex items-center justify-center order-1 lg:order-none">
          <Gauge chance={lockedChance ?? effectiveChance} rotation={rotation} spinning={spinning} fast={fast} result={result} cashback={cashback} phrase={phrase} onSpinEnd={finishSpin} />
        </div>

        <div className="blox-panel upgrade-preview-panel relative overflow-hidden h-[230px] sm:h-[300px] px-3 sm:px-4 pt-4 sm:pt-5 pb-3 sm:pb-4 flex flex-col order-3 lg:order-none" data-testid="target-panel">
          {target && <SlotItem item={target} disabled={spinning} onRemove={onClearTarget} testId="target-item" removeTitle={removeTitle} />}
          <div className="text-center">
            <div className="h-[30px] sm:h-[34px] flex items-center justify-center text-[12px] sm:text-[13px] font-bold leading-tight">{t("upgrade.target_title")}</div>
            <div className="text-[11px] text-[#7d8194] mt-1 flex items-center justify-center gap-1">
              <span className="inline-block min-w-[52px] text-right">{t("upgrade.target")}</span> <RobuxIcon size={11} /> <span className="tabular-nums inline-block min-w-[40px] text-left">{formatMoney(targetPrice)}</span>
            </div>
          </div>
          <div className="upgrade-slot slot-bg flex-1 flex items-center justify-center" data-testid="target-slot">
            <img src={REEL_FLY[1].image} alt="" className="slot-ghost" draggable={false} />
            <Logo size={92} muted testId="target-brand-icon" className="relative w-[64px] h-[64px] sm:w-[92px] sm:h-[92px]" />
          </div>
        </div>
        ...
        <div className="blox-panel min-h-11 px-4 py-1.5 flex flex-col justify-center gap-1.5 col-span-2 lg:col-span-1 order-5 lg:order-none" data-testid="balance-slider-block">
          ...
          <Slider
            ...
            onValueChange={(v) => { setLockedChance(null); setResult(null); setBet(Math.max(0, Math.min(maxBet, Number(v[0]) || 0))); }}
            className="w-full [&_[role=slider]]:h-4 [&_[role=slider]]:w-4 [&_[role=slider]]:bg-[#e6e8f0] [&_[role=slider]]:border-[#2a2c38] [&_[role=slider]]:shadow-[0_2px_6px_rgba(0,0,0,.6)] [&_.bg-primary]:bg-[linear-gradient(90deg,#5c6072,#9a9eb0)] [&_.bg-primary\/20]:bg-[#1c1d25]"
          />
        </div>

        <button
          className="blox-btn-dark h-11 w-full flex items-center justify-center gap-2 text-[14px] col-span-2 lg:col-span-1 order-4 lg:order-none"
