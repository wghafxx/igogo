...
const render = async (content) => act(async () => root.render(<MemoryRouter><LangProvider>{content}</LangProvider></MemoryRouter>));
...
test("support dialog copies the Telegram handle", async () => {
  await render(<SupportDialog request={{ kind: "support" }} onClose={jest.fn()} />);
  expect(byId("support-withdrawal-account")).toBeNull();
...
test.each([
  ...
  // Rejected requests live in the "completed" tab.
