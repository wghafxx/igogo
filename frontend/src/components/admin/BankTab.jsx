...
const ResetPanel = ({ data, onDone, askPin }) => {
  const reset = () => {
    const requestId = sessionStorage.getItem("admin-economy-reset") || crypto.randomUUID();
    askPin({
    danger: true,
    title: "Обнулить ВСЮ экономику и всех игроков?",
    description: `Банк ${formatMoney(data.bank)}, пул ${formatMoney(data.pool ?? 0)} и комиссия ${formatMoney(data.commission_profit ?? 0)} станут равны 0.\n\nУ ВСЕХ игроков обнулятся балансы и инвентари. Удалятся дроп, подарки и бонусы, история игр, побед и сливов, статистика RTP, пополнения, выводы, незавершённые заявки, история и резерв «Удачи». Вкладки «Банк» и «Игроки» начнут с нуля.\n\nСохраняются аккаунты, чаты, каталог, настройки игры и общий счётчик «Всего апгрейдов».\n\nДействие необратимо. Введите PIN-код.`,
    confirmLabel: "Обнулить всё",
    onConfirm: async (pin) => {
      try {
        sessionStorage.setItem("admin-economy-reset", requestId);
        await adminApi.bankReset(pin, requestId);
        sessionStorage.removeItem("admin-economy-reset");
        Object.keys(sessionStorage).filter((key) => key.startsWith("admin-coin-grant:")).forEach((key) => sessionStorage.removeItem(key));
        toast.success("Экономика обнулена: банк, игроки, дроп, статистика и удача. Общий счётчик апгрейдов сохранён.");
        onDone();
      } catch (e) {
        toast.error(e?.response?.data?.detail || "Ошибка");
        throw e;
  ...
  return (
    <div className="blox-panel p-4 space-y-2 border border-[#ff5c5c]/40" data-testid="bank-reset-panel">
      <div className="text-[13px] font-bold text-[#ff8a8a]">Полный сброс экономики</div>
      <div className="text-[11px] text-[#8e91a3]">Обнуляет банк, балансы и инвентари ВСЕХ игроков, дроп, подарки, игры, победы, сливы, фактический RTP и удачу. Сохраняются аккаунты, настройки и общий счётчик «Всего апгрейдов». Требуется PIN-код.</div>
...
export default function BankTab({ refreshKey = 0 }) {
  ...
  return (
    <div className="bank-tab space-y-4" data-testid="bank-tab">
