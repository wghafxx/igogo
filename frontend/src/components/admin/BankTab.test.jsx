...
test("full bank reset requires PIN and calls the reset endpoint", async () => {
  ...
  expect(byId("pin-dialog-title").textContent).toContain("ВСЮ экономику");
  expect(byId("pin-dialog-description").textContent).toContain("У ВСЕХ игроков обнулятся балансы и инвентари");
  expect(byId("pin-dialog-description").textContent).toContain("общий счётчик «Всего апгрейдов»");
  await confirmPin("1001");
  expect(adminApi.bankReset).toHaveBeenCalledWith("1001", "11223344-1122-4122-8122-112233445566");
  expect(byId("bank-balance").textContent).toContain("0");
  expect(byId("bank-pool").textContent).toContain("0");
});

test("retrying a timed out reset reuses its operation ID after reopening the dialog", async () => {
  adminApi.bankReset.mockRejectedValueOnce(new Error("timeout")).mockResolvedValueOnce({ ok: true });
  await click("bank-reset-button");
  await confirmPin();
  await click("pin-dialog-cancel");
  await click("bank-reset-button");
  await confirmPin();
  expect(adminApi.bankReset.mock.calls[1]).toEqual(adminApi.bankReset.mock.calls[0]);
  expect(sessionStorage.getItem("admin-economy-reset")).toBeNull();
});
