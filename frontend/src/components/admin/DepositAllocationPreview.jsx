...
export const MIN_DEPOSIT_RAP = 200;

export const DepositAllocationPreview = ({ depositId, chatId, rap, onReady }) => {
  ...
  useEffect(() => {
    ...
    if (!Number.isFinite(rap) || rap < MIN_DEPOSIT_RAP || rap > 1000000) return;
    const timer = setTimeout(() => {
      (chatId ? adminApi.chatDepositPreview(chatId, rap) : adminApi.depositPreview(depositId, rap)).then((result) => {
        if (alive) { setPlan(result); onReady(true); }
      }).catch(() => { if (alive) setError(true); });
    }, 300);
    return () => { alive = false; clearTimeout(timer); };
  }, [depositId, chatId, rap, retry, onReady]);
  if (rap < MIN_DEPOSIT_RAP || !Number.isFinite(rap) || rap > 1000000) return null;
