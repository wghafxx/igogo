...
export default function ChatSidebar({ q, setQ, status, setStatus, rows, found, selected, onPick, onOpenUser, counts, hasMore, total, onMore }) {
  return (
    <aside className="flex flex-col min-h-0 rounded-2xl bg-[#0f1015] border border-white/[0.05]" data-testid="admin-chat-sidebar">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-2 space-y-1" data-testid="admin-chat-list">
        ...
        {hasMore && <button type="button" onClick={onMore} className="w-full h-9 mt-1 rounded-lg bg-white/[0.05] text-[12px] font-bold text-[#a4a7b8] hover:text-white hover:bg-white/[0.1] transition-colors" data-testid="admin-chats-load-more">Показать ещё · {rows.length} из {total}</button>}
      </div>
    </aside>
  );
}
