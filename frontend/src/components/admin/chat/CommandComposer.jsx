...
export default function CommandComposer({ text, setText, busy, onSend, placeholder, loadCommands = adminApi.commands }) {
  ...
  return <footer className="p-3 border-t border-white/[0.05] space-y-2 shrink-0">
    ...
    <div className="flex items-end gap-2">
      <textarea ref={input} value={text} readOnly={busy} onChange={(e) => setText(e.target.value.slice(0, 2000))} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); send(); } }} rows={1} placeholder={placeholder} className="flex-1 min-w-0 min-h-[46px] max-h-32 resize-none rounded-xl bg-white/[0.05] px-4 py-3 text-[13px] outline-none focus:bg-white/[0.08] placeholder:text-[#6b6f84]" data-testid="admin-chat-input" />
      <button onClick={send} onMouseDown={(e) => e.preventDefault()} disabled={disabled} className="w-[46px] h-[46px] shrink-0 rounded-xl bg-[#ffb000] hover:bg-[#ffc233] disabled:opacity-40 text-black flex items-center justify-center" aria-label="Отправить" data-testid="admin-chat-send-button"><Send size={17} /></button>
    </div>
