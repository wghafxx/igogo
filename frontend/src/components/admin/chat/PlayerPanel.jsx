    ...
    <div className="mt-3 rounded-xl bg-white/[0.04] p-3 space-y-1">
      ...
      {user.roblox_link ? (
        <div className="flex items-center gap-2 min-w-0">
          <a href={user.roblox_link} target="_blank" rel="noopener noreferrer" className="text-[13px] font-bold text-[#9fd6ff] hover:text-white inline-flex items-center gap-1 truncate" data-testid="admin-chat-roblox-link">{user.roblox_nick} <ExternalLink size={12} /></a>
      ...
      <div className="space-y-3">
        {pending.map((d) => (
          <div key={d.id} className="rounded-xl bg-[#ffb000]/[0.08] border border-[#ffb000]/20 p-3 space-y-2" data-testid={`admin-chat-request-${d.id}`}>
            <div className="flex items-center justify-between text-[12px]"><span className="font-bold text-[#ffcf5a]">Заявка игрока · ~{formatMoney(d.expected_rap)} RAP</span><span className="text-[#8e91a3]">{ago(d.created_at)} назад</span></div>
        ...
        <Btn tone="success" size="lg" className="w-full" onClick={() => run(() => adminApi.chatDeposit(chatId, value, note), "Пополнение выполнено")} disabled={busy || !valid || !ready} data-testid="admin-chat-confirm-button">
          ...
          <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1">
            {withdrawals.map((w) => (
              <div key={w.id} className="flex items-center gap-3 rounded-xl bg-white/[0.04] px-3 py-2" data-testid="admin-chat-withdrawal-row">
                ...
                <span className="flex-1 min-w-0"><span className="block text-[12px] font-bold truncate">{w.item?.name}</span><span className="block text-[11px] text-[#8e91a3] truncate">{w.item?.type} · ждёт {ago(w.created_at)}{w.status === "cancelling" ? " · отмена…" : w.status === "paying" ? " · выдача…" : ""}</span></span>
