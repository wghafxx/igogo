...
import { X, Send, RotateCcw, LogOut } from "lucide-react";
...
const Composer = ({ onSend, disabled, autoFocus }) => {
  const { t } = useLang();
  ...
  return (
    <div className="chat-foot p-3 flex items-end gap-2 shrink-0">
      <div className="m-input m-input-sm flex-1 !h-auto min-h-[48px] py-2 items-end">
        <textarea autoFocus={autoFocus} value={text} onChange={(e) => setText(e.target.value.slice(0, 2000))} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); submit(); } }} rows={1} placeholder={t("chat.placeholder")} className="flex-1 min-w-0 max-h-32 resize-none bg-transparent outline-none text-[14px] font-medium placeholder:text-white/30 placeholder:font-normal leading-[22px] py-[5px]" data-testid="chat-input" />
        <button onClick={submit} disabled={busy || !text.trim() || disabled} className="m-iconbtn shrink-0" data-active={text.trim() ? "true" : "false"} aria-label={t("chat.send")} data-testid="chat-send-button">
          <Send size={16} />
        </button>
      </div>
    </div>
    ...
    <>
      {chat.open && (
        <div className="chat-surface fixed z-[60] bottom-24 right-3 sm:right-6 w-[calc(100vw-24px)] sm:w-[440px] h-[min(680px,calc(100vh-120px))] flex flex-col overflow-hidden animate__animated animate__fadeInUp animate__faster" data-testid="live-chat-panel">
          ...
          {chat.chat ? (
            <>
              <ChatMessages messages={chat.messages} mine="user" />
              {closed ? (
            ...
            <>
              ...
              <Composer onSend={chat.send} autoFocus />
            </>
          )}
        </div>
      )}
