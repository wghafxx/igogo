...
const PriceInput = ({ value, onChange, placeholder, testId, ariaLabel }) => (
  <div className="relative min-w-0 flex-1">
    ...
    <input
      ...
      className="h-9 w-full min-w-0 pl-6 pr-1 rounded-lg border border-[#383838] bg-transparent text-[12px] text-white placeholder:text-[#999999] outline-none focus:ring-1 focus:ring-white"
      data-testid={testId}
    />
  </div>
);
...
export default function SkinCatalog({ pageSize, title, headerAction, renderItem, testPrefix = "" }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => api
      .shop(params, { signal: controller.signal })
      .then((d) => {
        if (!alive) return;
        setItems(d.items || []);
  ...
  return (<>
        ...
        <SkinGrid pageSize={pageSize} testId={`${testPrefix}shop-grid`} overlay={
          loading ? <div className="rounded-lg bg-[#0d0e12] p-4 text-sm text-[#8e91a3]" data-testid={`${testPrefix}shop-loading`}>{t("skins.loading")}</div>
            : error ? <div className="rounded-lg bg-[#0d0e12] p-4 text-sm" data-testid={`${testPrefix}shop-error`}>{t("skins.catalog_fail")}<button className="block mx-auto mt-3 text-white underline" data-testid={`${testPrefix}shop-retry`} onClick={() => setRetry((v) => v + 1)}>{t("skins.retry")}</button></div>
