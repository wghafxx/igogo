  ...
  return (
    <div className="space-y-3" data-testid="my-requests">
      <div className="flex items-center justify-between">
        <div className="text-[13px] font-bold">{t("requests.title")}</div>
      ...
      {items.length === 0 && <div className="h-[140px] flex items-center justify-center text-[13px] text-[#5f6377] rounded-xl bg-[#0f1015]" data-testid="my-requests-empty">{t("requests.empty")}</div>}
      ...
      <div className="text-[11px] text-[#5f6377] leading-snug">{t("requests.hint")}</div>
    </div>
  );
}
