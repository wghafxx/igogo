...
export default function SbpTopUp({ onDone }) {
  ...
  const goToSupport = async (text) => {
    ...
    setBusy(true);
    try {
      const created = await api.createChat({ kind: "support", text });
  ...
  return (
    <div data-testid="topup-rub-step">
      ...
      {method === "card" && (
        <div className="m-foot space-y-3">
          <div className="rounded-[12px] m-note-info px-4 py-3 text-[12px] leading-relaxed" data-testid="card-info">{t("rub.card_text")}</div>
          <button onClick={() => goToSupport(t("rub.chat_card"))} disabled={busy} className="m-cta" data-testid="card-support-button">
            {!authUser ? t("rub.login") : t("rub.card_btn")}
          </button>
