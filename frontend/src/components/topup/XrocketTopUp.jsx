...
const stateKey = { creating: "waiting", awaiting_payment: "waiting", processing: "processing", confirmed: "confirmed", expired: "expired", cancelled: "cancelled", payment_error: "failed" };
...
export default function XrocketTopUp({ onChanged }) {
  ...
  const pay = async () => {
    ...
    if (!valid || !info?.enabled || busy) return;
    const account = authUser.session_id;
    const identity = `${account}:${rub}:${currency}`;
    if (requestRef.current?.identity !== identity) requestRef.current = { identity, id: crypto.randomUUID() };
    setBusy(true);
    setError("");
    try {
      const result = await api.createXrocketInvoice({ request_id: requestRef.current.id, amount_rub: num, currency });
      if (accountRef.current !== account) return;
      setInvoice(result);
      loadHistory();
      changedRef.current?.();
  ...
  return <div className="space-y-4" data-testid="xrocket-topup">
    ...
    {invoice ? <div className="rounded-xl border border-[#00a2ff]/30 bg-[#0f1015] p-4 space-y-4" data-testid="xrocket-invoice">
      ...
      {PENDING.includes(invoice.status) && <>
        {invoice.expires_at && <p className="text-[11px] text-[#8e91a3]">{t("xrocket.until")} {parseServerDate(invoice.expires_at).toLocaleString(lang === "en" ? "en-US" : "ru-RU", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</p>}
        {invoice.invoice_url && <a href={invoice.invoice_url} target="_blank" rel="noopener noreferrer" className="h-11 flex items-center justify-center gap-2 rounded-xl bg-[#00a2ff] hover:bg-[#20afff] font-bold text-[13px] transition-colors" data-testid="xrocket-open">{t("xrocket.open")} ↗</a>}
      ...
      <button onClick={() => { setInvoice(null); setError(""); requestRef.current = null; loadHistory(); }} disabled={busy} className="text-[12px] text-[#00a2ff] hover:text-white disabled:opacity-40" data-testid="xrocket-new">{t("xrocket.new")}</button>
    </div> : <>
    ...
    {error && <p role="alert" className="text-[12px] text-[#ff8a8a] leading-relaxed">{error}</p>}
    ...
    {!invoice && history.length > 0 && <div className="space-y-1.5"><h4 className="text-[12px] font-bold">{t("xrocket.history")}</h4>{history.slice(0, 5).map((row) => <button key={row.id} onClick={() => { setInvoice(row); setError(""); }} className="w-full flex items-center justify-between gap-3 p-2.5 bg-[#15161c] hover:bg-[#252630] rounded-lg text-[11px] text-left" data-testid="xrocket-history-item"><b>{formatMoney(row.amount_rub)} ₽ · {row.currency}</b><span className={row.status === "confirmed" ? "text-[#61d899]" : "text-[#8e91a3]"}>{t(`xrocket.${stateKey[row.status] || "waiting"}`)}</span></button>)}</div>}
