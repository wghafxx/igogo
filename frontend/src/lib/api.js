...
export const adminApi = {
  ...
  bankReset: (pin) => adminHttp.post(`/admin/bank/reset`, { pin }).then((r) => r.data),
  ...
  chats: (status, q) => adminHttp.get(`/admin/chats`, { params: q ? { status, q } : { status } }).then((r) => r.data),
  search: (q) => adminHttp.get(`/admin/search`, { params: { q } }).then((r) => r.data),
  chatDeposit: (id, rap, note) => adminHttp.post(`/admin/chats/${encodeURIComponent(id)}/deposit`, { rap, note }).then((r) => r.data),
