...
export default function AdminPage() {
  ...
  // The chats tab already refreshes the summary with its list and reports it here; poll only elsewhere.
  useEffect(() => {
    if (!authed || tab === "chats") return undefined;
    ...
    <div className="min-h-screen bg-[#0d0e12] text-white" data-testid="admin-page">
      ...
      <main className={`${tab === "chats" ? "max-w-[1600px]" : "max-w-[1100px]"} mx-auto px-4 py-6 space-y-4`}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {tab === "chats" && <TelegramCard />}
        </div>

        {tab === "chats" && <ChatsTab refreshKey={refreshKey} onSummary={setChatSummary} />}
        {tab === "staff" && <StaffTab refreshKey={refreshKey} />}
