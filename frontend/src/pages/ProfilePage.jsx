...
export default function ProfilePage() {
  ...
  return (
    <div className="profile-page max-w-[1000px] mx-auto space-y-4" data-testid="profile-page">
      {loadError && <div className="blox-panel p-4 text-sm text-[#ff8a8a]" data-testid="profile-load-error">{t("profile.load_fail")} <button onClick={load} className="text-[#00a2ff]" data-testid="profile-retry">{t("common.retry")}</button></div>}
            ...
            <Stat title={t("profile.best")} icon={SparklesIcon} testId="profile-best-drop">
              {data?.best_drop ? (
                <div className="flex items-center gap-3" style={{ "--rarity": rarityColor(data.best_drop.item_rarity) }}>
                  <img src={skinImg(data.best_drop.item_image)} alt="" width={56} height={56} decoding="async" className="w-14 h-14 object-contain" />
                  <div className="min-w-0">
        ...
        <div className="blox-panel fade-up" style={{ animationDelay: "80ms" }}>
          ...
          <div className="p-3 min-h-[220px]">
            ...
            {tab === "items" && (
              <div className="profile-history-list space-y-1.5" data-testid="profile-item-history">
                ...
                {(data?.item_history || []).map((h) => (
                  <div key={h.id} className="min-h-12 py-2 px-3 rounded-lg bg-[#0f1015] flex flex-wrap items-center gap-3 text-[12px]" style={{ boxShadow: `inset 3px 0 0 ${rarityColor(h.item?.rarity)}` }}>
                    {h.item?.image && <img src={skinImg(h.item.image)} alt="" width={36} height={36} loading="lazy" decoding="async" className="w-9 h-9 object-contain" />}
            ...
            {tab === "payments" && (
              <div className="space-y-1.5" data-testid="profile-payments">
                ...
                {deposits.map((d) => (
                  <div key={d.id} className="profile-payment-item min-h-12 px-3 py-2 rounded-lg bg-[#0f1015] flex flex-wrap items-center gap-3 text-[12px]" data-testid="profile-payment-item">
            ...
            {tab === "games" && (
              <div className="profile-history-list space-y-1.5" data-testid="profile-games-history">
