...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    setData(null);
    setError(null);
    api.publicProfile(discordId).then((value) => { if (alive) setData(value); }).catch((e) => { if (alive) setError(e?.response?.data?.detail || t("profile.load_fail")); });
    return () => { alive = false; };
  ...
  const copyLink = async () => {
    ...
    } catch { toast.error(t("pub.copy_fail")); }
  };
  ...
  if (error)
    return (
      <div className="max-w-[1000px] mx-auto blox-panel p-10 text-center" data-testid="public-profile-error">
        ...
        <Link to="/" className="inline-block mt-4 text-[#00a2ff] text-[13px] font-bold" data-testid="public-profile-home-link">{t("pub.home")}</Link>
      </div>
    );
    ...
    <div className="max-w-[1000px] mx-auto space-y-4" data-testid="public-profile-page">
      <div className="blox-panel p-3 grid grid-cols-1 md:grid-cols-[320px_1fr] gap-3 fade-up">
        <div className="rounded-xl bg-[#0f1015] p-4 flex flex-col gap-3">
          ...
          <button onClick={copyLink} className="h-9 rounded-lg bg-[#1c1d25] hover:bg-[#22242e] text-[12px] font-bold text-[#c9ccd8] flex items-center justify-center gap-2 transition-colors" data-testid="public-profile-copy-link">
            <LinkIcon size={13} /> {t("pub.copy")}
          </button>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Stat title={t("pub.upgrades")} icon={Logo} testId="public-profile-upgrades">
            <div className="text-[22px] font-black leading-none">{s.upgrades}</div>
            <div className="text-[11px] text-[#8e91a3]">{t("pub.wins")} {s.wins}</div>
          </Stat>
          <Stat title={t("pub.inventory")} icon={BoxesIcon} testId="public-profile-inventory">
            <div className="text-[12px] text-[#8e91a3]">{s.inventory_count} {t("pub.items_suffix")}</div>
            <div className="font-bold flex items-center gap-1">{formatMoney(s.inventory_value)} <RobuxIcon size={11} /></div>
          </Stat>
          <Stat title={t("pub.withdrawn")} icon={SendIcon} testId="public-profile-withdrawn">
            <div className="text-[12px] text-[#8e91a3]">{s.withdrawn_count} {t("pub.items_suffix")}</div>
            <div className="font-bold flex items-center gap-1">{formatMoney(s.withdrawn_sum)} <RobuxIcon size={11} /></div>
          </Stat>
          <Stat title={t("pub.best")} icon={SparklesIcon} testId="public-profile-best-drop">
            {data.best_drop ? (
              <div className="flex items-center gap-2" style={{ "--rarity": rarityColor(data.best_drop.item_rarity) }}>
                <img src={skinImg(data.best_drop.item_image)} alt="" width={40} height={40} decoding="async" className="w-10 h-10 object-contain" />
                <div className="min-w-0">
              ...
              <div className="text-[12px] text-[#5f6377]">{t("pub.best_empty")}</div>
            )}
          </Stat>
        </div>
      </div>
      ...
      <div className="blox-panel fade-up" style={{ animationDelay: "80ms" }}>
        <div className="px-3 py-2.5 border-b border-[#15161b] font-bold text-[13px] flex items-center gap-2">
          <SparklesIcon size={14} className="text-[#ffb000]" /> {t("pub.recent")}
        </div>
        <div className="p-3 min-h-[160px]">
          {data.drops.length ? (
            ...
            <div className="h-[140px] flex items-center justify-center text-[13px] text-[#5f6377]" data-testid="public-profile-drops-empty">{t("pub.recent_empty")}</div>
          )}
        </div>
      </div>
    </div>
