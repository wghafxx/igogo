...
const RU = [
  ...
  ["3. Апгрейды и трейды", [
    ...
    "3.5. Пополнение скинами производится через трейд в Roblox: сначала добавьте нас в друзья, затем отправьте скины и оставьте заявку на сайте. Денежные способы пополнения описаны в разделе 8.",
    ...
    "3.7. Принимаются скины от 200 RAP. Скины дешевле 200 RAP не зачисляются на баланс и не возвращаются — вы можете потерять такой скин. Будьте внимательны.",
    "3.8. Промокод даёт бонус к сумме зачисления (например +10%). При пополнении скинами бонус считается после вычета 20%; при оплате xRocket — от RAP по тарифу без комиссии сайта. Условия промокода фиксируются при создании заявки или счёта.",
    "3.9. После подтверждения пополнения скинами рассчитанная сумма распределяется на покупку до пяти разных предметов из каталога Сервиса. Подбор стремится к близким долям, но зависит от доступных цен. Неиспользованный остаток зачисляется на баланс. Если суммы недостаточно для одного предмета, она зачисляется на баланс целиком. Стоимость выданных предметов вместе с остатком не превышает сумму после комиссии и промокода.",
  ]],
...
const EN = [
  ...
  ["3. Upgrades and trades", [
    ...
    "3.5. Skin top-ups use Roblox trades: add us as a friend, send the skins, and submit a request on the website. Monetary top-up methods are described in section 8.",
    ...
    "3.7. Skins of 200 RAP and up are accepted. Cheaper skins are not credited and not returned — you may lose such a skin. Be careful.",
    "3.8. A promo code gives a bonus to the credited amount (e.g. +10%). For skins it applies after the 20% deduction; for xRocket it applies to RAP at the stated rate with no site fee. Promo terms are fixed when the request or invoice is created.",
    "3.9. After a skin deposit is confirmed the calculated amount is spent on up to five different items from the Service catalog. Matching aims for close shares but depends on available prices. The unused remainder goes to balance. If the amount is not enough for a single item, it goes to balance in full. The value of issued items plus the remainder never exceeds the amount after fee and promo.",
  ]],
  ["4. Item rarity", [
...
export default function TosPage() {
  ...
  return (
    <div className="tos-page max-w-[860px] mx-auto" data-testid="tos-page">
