...
## Сессия 5 (2026-09-23): поддержка на языке игрока, скриншоты, DonationAlerts + Telegram
Запрос: автоответы чата на английском по выбранному языку; скриншоты в поддержку (≤1 МБ, удаление через 3 дня, без спама, БД 5 ГБ на Railway);
оплата в валюте — автоинструкция DonationAlerts (https://www.donationalerts.com/r/bloxgrade), кнопка «Я оплатил — проверить» один раз,
уведомление в Telegram-бот владельцу (id 5095885655), подтверждение/отклонение только из этого аккаунта.
Сделано:
- chat_texts.py: ru/en автотексты; фронт шлёт X-Lang, chat.lang обновляется при создании/сообщении/открытии.
- chat_attachments.py: коллекция chat_attachments (Binary, TTL expires_at 3 дня), только Discord-аккаунты, 1 МБ, 5/сутки, 20 с между загрузками,
  общий лимит CHAT_ATTACHMENTS_MAX_MB (1024), проверка сигнатуры PNG/JPEG/WebP; браузер сжимает в WebP ≤1600px.
- donation_payments.py: заявка (payment_method donationalerts, код BG-XXXXXX), инструкция в чате, claim один раз (атомарно),
  Telegram: карточка + скрин игрока, кнопки «Пришло X ₽ → Y RAP» (для RUB) / «Ввести сумму ₽» (force_reply) / «Отклонить»,
  всегда второе подтверждение; только from.id == TELEGRAM_ADMIN_ID; секрет вебхука; дедуп update_id; повтор отправки раз в минуту.
  Зачисление — существующий confirm_deposit (банк не менялся), курс сайта 1 RAP = 0,50 ₽ (cryptobot RAP_RUB_RATE).
- /admin → вкладка Чаты: карточка Telegram (статус, «Подключить бота к этому сайту» = setWebhook на PUBLIC_APP_URL).
- env: TELEGRAM_BOT_TOKEN, TELEGRAM_ADMIN_ID, TELEGRAM_WEBHOOK_AUTO, DONATIONALERTS_URL, CHAT_ATTACHMENTS_MAX_MB.
- Сессия 6: /admin → «Проверочное сообщение» (da:test:*, ничего не зачисляет); фраза доната из 60 (da_phrases.py), не повторяется среди 5 последних и не совпадает с ожидающими заявками; игрок пишет её в сообщение DonationAlerts, видна в Telegram и админке.
Важно: владелец должен нажать /start в @BloxGrade_bot, иначе Telegram не даёт боту писать. После деплоя на Railway — нажать «Подключить бота» в /admin.

## Сессия 4 (2026-09-23): устранение замечаний код-ревью (commit 927d806)
Исходный запрос: устранить все ошибки из объединённого отчёта ревью, НЕ трогая систему выпадения, шансы и банк (главное — не сломать банк).
Не менялось: win_chance/shown_chance/landing/losing_roll, bank_add/bank_state/pool/RTP, confirm_deposit/settle_deposit, /upgrade (кроме проверки токена), rain.
Сделано:
- Безопасность: SPA-файлы только внутри static (safe_static_file, resolve+is_relative_to); JWT session_version (claim sv), logout отзывает все копии токена; локальный выход всегда; в админке пометка «Roblox не подтверждён».
- Надёжность: вывод скинов — намерение (status reserving + маркер withdrawal_ops в том же update) + восстановление при старте; идемпотентный admin chat deposit (request_id → deposits.admin_request_id unique); CryptoBot — восстановление счёта по payload/ID из вебхука; снимок получателя в withdrawal.recipient + recipient_changed; shop_receipts: inline ≤20 + архив db.shop_receipts.
- Чат: уникальный чат на владельца (owner_unique), переписка архивируется (chat_messages_archive), admin_list с пагинацией {items,has_more,total}, admin_summary агрегатом, mark_read условно, инкрементальная загрузка сообщений (after, $gte + dedupe), chatId из события open-live-chat, защита от устаревших ответов.
- Нагрузка: usePolling (без наложений, пауза при скрытой вкладке), лента 5с только на главной (20с на прочих), микрокэш live-drops 2с, admin last_seen раз в 60с, TTL presence 24ч, bulk-синхронизация каталога по хэшу, ChatsTab debounce 300мс + seq, одна сводка чатов, will-change убран.
- Архитектура: authUser — единственный источник пользователя, контекст сессии через useMemo, PromoInput вынесен (цикл импортов разорван), lib/events.js, каталог в catalog.py, CSS без data-testid, дубли .skin-card сведены.
- Уборка: удалены public/fixes/rapid-click.patch, копии в test_reports, записи .emergent/recordings, пустой tests/, ReceiverStep, ChatDepositPanel, ChatWithdrawalsPanel, старые фоны bg-train/bg-dust2(+blur) и этап скрипта, memory/.test_token, 37 неиспользуемых ui-компонентов и ~38 неиспользуемых npm-пакетов (react-query, swr, framer-motion, dayjs, date-fns, recharts, lodash, cra-template…).
- Сборка/тесты: yarn.lock, закреплённые requirements.txt (runtime) + requirements-test/dev, Dockerfile (--frozen-lockfile, только *.py, USER app), .dockerignore; pytest python_files=test_*.py, маркер live (-m "not live"); устаревшие тесты исправлены/удалены. Итог: backend 297 passed (-m "not live"), frontend jest 97/97, build OK.
Оставлено сознательно: разбиение server.py на модули не делалось глубже каталога (рядом код банка/прокрута); gram.png/ton.png — разные коды валют; PointerEventsGuard; craco-адаптеры.
Backlog: P1 — вынести auth/чат-роуты server.py в APIRouter-модули; P2 — страница архива переписки в админке; P2 — проверка владения Roblox (код в описании профиля).
