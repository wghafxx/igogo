...
- Дополнение: сотрудник видит выводы игрока (карточка «Вывод скинов» только для чтения — без отмены и «Всё выдано», подсказка позвать главного) и открытые денежные пополнения (СБП/DonationAlerts: сумма, код, статус «Оплатил») с подсказкой объяснить оплату через донат. Действия по выводу остаются только у владельца (403 для сотрудника).
